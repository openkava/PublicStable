...
FABRIC.RT.Vec3 = function() {
  ...
  else throw'new FABRIC.RT.Vec3: invalid arguments';
  };
...
FABRIC.RT.Vec3.prototype = {
  ...
  divide: function(v) {
    if( Math.verboseLogFunction ) {
      Math.checkDivisor(v.x, 'FABRIC.RT.Vec3.divide v.x');
      Math.checkDivisor(v.y, 'FABRIC.RT.Vec3.divide v.y');
      Math.checkDivisor(v.z, 'FABRIC.RT.Vec3.divide v.z');
    }
    return new FABRIC.RT.Vec3(this.x / v.x, this.y / v.y, this.z / v.z);
  },

  divideScalar: function(s) {
    Math.checkDivisor(s, 'FABRIC.RT.Vec3.divideScalar');
  ...
  inverse: function() {
    if( Math.verboseLogFunction ) {
      Math.checkDivisor(this.x, 'FABRIC.RT.Vec3.inverse this.x');
      Math.checkDivisor(this.y, 'FABRIC.RT.Vec3.inverse this.y');
      Math.checkDivisor(this.z, 'FABRIC.RT.Vec3.inverse this.z');
    }
  ...
  unit: function() {
    var len = this.length();
    Math.checkDivisor(len, 'FABRIC.RT.Vec3.unit');
  ...
  setUnit: function() {
    var len = this.length();
    Math.checkDivisor(len, 'FABRIC.RT.Vec3.setUnit');
    var invLen = 1.0 / len;
    this.x *= invLen;
    this.y *= invLen;
    this.z *= invLen;
...
FABRIC.appendOnCreateContextCallback(function(context) {
  context.RegisteredTypesManager.registerType('Vec3', {
    ...
    klBindings: {
      filename: 'Vec3.kl',
      sourceCode: FABRIC.loadResourceURL('FABRIC_ROOT/RT/Vec3.kl')
    }
  });
});
