...
for f in "$@"; do
  ...
  LD_LIBRARY_PATH=build/ $VALGRIND_CMD ../../build/$BUILD_OS/$BUILD_ARCH/$BUILD_TYPE/Fabric/Clients/CLI/fabric --load="$WRAPPERS_FILE" --exts="$EXTS_DIR" $f 2>&1 | grep -v '^\[FABRIC\] .*Extension registered' | grep -v '^\[FABRIC\] .*Searching extension directory' | grep -v '^\[FABRIC\] .*unable to open extension directory' | $OUTPUT_FILTER >$TMPFILE
