...
namespace Fabric
{
  ...
  namespace DG
  {
    ...
    Util::Encoder &Container::encode( Util::Encoder &encoder ) const
    {
      encoder.put( sk_encodingCookie );
      encoder.put( sk_encodingVersion );
      
      size_t count = m_count;
      encoder.putSize( count );
      
      size_t numMembers = m_members.size();
      encoder.putSize( numMembers );
      ...
      for ( Members::const_iterator it=m_members.begin(); it!=m_members.end(); ++it )
      {
        ...
        void const *data = member->getElementData(0);
        for ( size_t i=0; i<m_count; ++i )
        {
          desc->encode( encoder, data );
          data = (uint8_t const *)data + elementSize;
        }        
      }
    ...
    Util::Decoder &Container::decode( Util::Decoder &decoder )
    {
      uint64_t encodingCookie;
      decoder.get( encodingCookie );
      if ( encodingCookie != sk_encodingCookie )
        throw Exception( "unknown encoding cookie %016lX", encodingCookie );

      uint32_t encodingVersion;
      decoder.get( encodingVersion );
      if ( encodingVersion != sk_encodingVersion )
        throw Exception( "unknown encoding version %08X", encodingVersion );
      ...
      for ( size_t i=0; i<numMembers; ++i )
      {
        try
        {
          ...
          try
          {
            ...
            if ( it != m_members.end() )
              member = it->second;
            else throw Exception( "missing member " + _(name) );
              
            RC::ConstHandle<RT::Desc> desc = member->getDesc();
            void *data = member->getElementData(0);
            
            for ( size_t i=0; i<count; ++i )
            {
              desc->decode( decoder, data );
              data = (uint8_t *)data + elementSize;
            }
          }
          catch ( Exception e )
          {
    ...
    Util::Encoder &Container::encodeSlice( Util::Encoder &encoder, size_t index ) const
    {
      if ( index >= m_count )
        throw Exception("index out of range");

      encoder.put( sk_sliceEncodingCookie );
      encoder.put( sk_sliceEncodingVersion );
      ...
      for ( Members::const_iterator it=m_members.begin(); it!=m_members.end(); ++it )
      {
        ...
        desc->encode( encoder, member->getElementData(index) );
      }
      
      return encoder;
    }
    
    Util::Decoder &Container::decodeSlice( Util::Decoder &decoder, size_t index )
    {
      if ( index >= m_count )
        throw Exception("index out of range");
      ...
      uint64_t encodingCookie;
      decoder.get( encodingCookie );
      if ( encodingCookie != sk_sliceEncodingCookie )
        throw Exception( "unknown encoding cookie %016lX", encodingCookie );

      uint32_t encodingVersion;
      decoder.get( encodingVersion );
      if ( encodingVersion != sk_sliceEncodingVersion )
        throw Exception( "unknown encoding version %08X", encodingVersion );
      ...
      for ( size_t i=0; i<numMembers; ++i )
      {
        ...
        if ( it != m_members.end() )
          member = it->second;
        else throw Exception( "missing member '" + name + "'" );
          
        RC::ConstHandle<RT::Desc> desc = member->getDesc();
        desc->decode( decoder, member->getElementData(index) );
      }
