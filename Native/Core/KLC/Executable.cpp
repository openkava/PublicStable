...
namespace Fabric
{
  namespace KLC
  {
    ...
    RC::Handle<ArrayTransformOperator> Executable::resolveArrayTransformOperator( std::string const &operatorName ) const
    {
      void (*functionPtr)( ... ) = 0;
      RC::ConstHandle<AST::Operator> astOperator;
      ...
      if ( !m_diagnostics.containsError() )
      {
        ...
        if ( !functionPtr )
          throw Exception( "array transform operator " + _(operatorName) + " not found" );
      }
      
      return ArrayTransformOperator::Create(
        this,
        astOperator,
        functionPtr
        );
      ...
      )
    {
      if ( cmd == "getAST" )
        ...
        jsonExecResolveValueTransformOperator( arg, resultJAG );
      else if ( cmd == "resolveArrayTransformOperator" )
        jsonExecResolveArrayTransformOperator( arg, resultJAG );
    ...
    void Executable::jsonExecResolveArrayTransformOperator(
      RC::ConstHandle<JSON::Value> const &arg,
      Util::JSONArrayGenerator &resultJAG
      )
    {
      ...
      resolveArrayTransformOperator( operatorName )->reg( m_gcContainer, id_ );
    }
  }
}
